...
describe('Recommendation Service', () => {
  ...
  describe('analyzeUserPreferences', () => {
    it('should return null when no user activity exists', async () => {
      ...
      getProductViewHistory.mockResolvedValue([]);
  ...
  describe('getRecommendedProducts', () => {
    it('should return popular products when no user preferences exist', async () => {
      ...
      // Mock getSwipeHistory to return empty array
      getSwipeHistory.mockResolvedValue([]);
    ...
    it('should return products based on user preferences', async () => {
      ...
      fetchProductsByTags.mockResolvedValue([...mockYesProducts, mockClickedProducts[0]]);
    ...
    it('should exclude already swiped products', async () => {
      ...
      // Mock fetchProductsByTags
      fetchProductsByTags.mockResolvedValue([mockYesProducts[1], mockClickedProducts[0]]);
  ...
  describe('getRecommendationsByCategory', () => {
    it('should return recommendations grouped by category', async () => {
      ...
      // Setup for fetchProductsByCategoryAndTags and fetchProductsByCategory
      // We'll mock implementation behavior using Jest's mockImplementation
      
      // Create a mock for the internal fetchProductsByCategoryAndTags function
      const mockFetchByCategoryAndTags = jest.fn()
        .mockImplementation((category, tags, limit, excludeIds) => {
          // Return different products based on category
          if (category === 'tops') {
            return Promise.resolve([mockYesProducts[0]]);
          } else if (category === 'bottoms') {
            return Promise.resolve([mockViewedProducts[0]]);
          } else if (category === 'outerwear') {
            return Promise.resolve([mockNoProducts[0]]);
          } else {
      ...
      jest.spyOn(recommendationModule, 'getRecommendationsByCategory')
        .mockImplementation(async (userId, categories, limit) => {
          const result = {};
          for (const category of categories) {
            result[category] = await mockFetchByCategoryAndTags(category);
          }
          return result;
        });
