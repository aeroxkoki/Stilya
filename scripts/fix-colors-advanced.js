...
const colorMapping = {
  ...
  '#000080': 'theme.colors.primary',
  '#007AFF': 'theme.colors.primary',
  '#FF0000': 'theme.colors.error',
  '#FF6B6B': 'theme.colors.error',
  '#4CAF50': 'theme.colors.success',
  '#00C853': 'theme.colors.success',
  '#FFC107': 'theme.colors.warning',
  '#ddd': 'theme.colors.border',
  '#f8f8f8': 'theme.colors.surface',
  '#F8F8F8': 'theme.colors.surface',
  '#3498db': 'theme.colors.status.info',
  '#e74c3c': 'theme.colors.error',
  '#2ecc71': 'theme.colors.success',
  '#f39c12': 'theme.colors.warning',
  '#95a5a6': 'theme.colors.text.hint',
  '#34495e': 'theme.colors.text.secondary',
  '#2c3e50': 'theme.colors.text.primary',
  '#ecf0f1': 'theme.colors.surface',
  '#bdc3c7': 'theme.colors.text.disabled',
  '#7f8c8d': 'theme.colors.text.secondary',
};

// コンポーネントのスタイルをインラインスタイルに変換する
function transformStyleSheetColors(content, stylesToModify) {
  let modifiedContent = content;
  let changes = [];

  stylesToModify.forEach(({ styleName, colorProps }) => {
    colorProps.forEach(({ property, value, themeColor }) => {
      // スタイルの使用箇所を検索
      const styleUsageRegex = new RegExp(`styles\\.${styleName}(?![\\w])`, 'g');
      
      modifiedContent = modifiedContent.replace(styleUsageRegex, (match) => {
        changes.push({
          style: styleName,
          property: property,
          oldValue: value,
        ...
        // インラインスタイルとして追加
        return `[styles.${styleName}, { ${property}: ${themeColor} }]`;
      });
    });
  });
...
// StyleSheet内のハードコーディングされた色を見つける
function findHardcodedColorsInStyleSheet(content) {
  const styleSheetMatch = content.match(/const\s+styles\s*=\s*StyleSheet\.create\s*\(([\s\S]*?)\}\s*\);/);
  ...
  while ((match = styleRegex.exec(stylesContent)) !== null) {
    ...
    colorPatterns.forEach(pattern => {
      const colorMatches = styleContent.match(pattern);
      if (colorMatches) {
    ...
    if (colorProps.length > 0) {
      results.push({ styleName, colorProps });
    }
  }
...
function processFile(filePath) {
  ...
  // StyleSheet内のハードコーディングされた色を処理
  const stylesToModify = findHardcodedColorsInStyleSheet(content);
  if (stylesToModify.length > 0) {
    const transformResult = transformStyleSheetColors(content, stylesToModify);
  ...
  if (modified && !hasThemeVariable) {
    ...
    if (componentMatch) {
      ...
      // すでにuseStyleの呼び出しがあるか確認
      const afterComponent = content.slice(insertPosition);
      if (!afterComponent.includes('const { theme } = useStyle();')) {
        content = content.slice(0, insertPosition) + 
