// Supabaseダッシュボードで実行するSQLを生成するスクリプト
...
// 確認クエリ
console.log('-- 4. 挿入結果を確認');
console.log('SELECT COUNT(*) as total_products FROM external_products;');
console.log('SELECT * FROM external_products LIMIT 5;');
console.log('');

console.log('-- 5. 必要に応じてRLSを再度有効化（本番環境では推奨）');
console.log('-- ALTER TABLE external_products ENABLE ROW LEVEL SECURITY;');
console.log('');

console.log('/*');
console.log('注意事項:');
console.log('1. 上記のSQLをSupabaseダッシュボードのSQL Editorで実行してください');
console.log('2. 実行前に必ずRLSを無効化してください（ステップ1）');
console.log('3. 商品データが正しく挿入されたら、アプリを再起動してください');
console.log('4. 本番環境では適切なRLSポリシーを設定してからRLSを有効化してください');
console.log('*/');
